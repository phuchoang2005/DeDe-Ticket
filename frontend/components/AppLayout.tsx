...
/** Application chrome: header, desktop nav, mobile drawer, notification bell, footer. */
      ...
      <header className="bg-white border-b border-line sticky top-0 z-30">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-3 flex items-center gap-3 sm:gap-6">
          ...
          <nav className="hidden md:flex gap-1 items-center">
            <NavLink href="/" end className={desktopNavLink}>Trang chủ</NavLink>
            <NavLink href="/events" className={desktopNavLink}>Sự kiện</NavLink>
            {user && <NavLink href="/tickets" className={desktopNavLink}>Vé của tôi</NavLink>}
            {user && <NavLink href="/notifications" className={desktopNavLink}>Thông báo</NavLink>}
            {user && (
              <Link href="/feedback" className="ml-1 text-xs font-semibold px-3 py-1.5 rounded-full border border-brand-200 bg-brand-50 text-brand-700 hover:bg-brand-100 transition-colors">
                Phản hồi
              </Link>
            )}
            {user && (user.roles?.includes('ADMIN') || user.roles?.includes('ORGANIZER')) && (
              <>
                <NavLink href="/admin/events" className={desktopNavLink}>Quản trị sự kiện</NavLink>
                <NavLink href="/admin/analytics" className={desktopNavLink}>Báo cáo</NavLink>
                <NavLink href="/admin/feedback" className={desktopNavLink}>Phản hồi KH</NavLink>
          ...
          <div className="ml-auto flex items-center gap-1 sm:gap-3">
            {user ? (
                  ...
                  {accountOpen && (
                    <>
                      ...
                      <div className="absolute right-0 mt-2 w-56 bg-white border border-line rounded-xl shadow-pop z-20 overflow-hidden">
                        ...
                        <Link href="/tickets" onClick={() => setAccountOpen(false)}
              ...
              <>
                <Link href="/login" className="hidden sm:inline-flex text-sm font-medium text-ink-muted hover:text-ink px-3 py-2">
                  Đăng nhập
                </Link>
        ...
        <>
          ...
          <aside className="md:hidden fixed top-0 left-0 bottom-0 w-72 max-w-[85vw] bg-white z-50 shadow-pop p-4 flex flex-col">
            ...
            <nav className="space-y-1">
              <NavLink href="/" end className={drawerNavLink}>🏠 Trang chủ</NavLink>
              <NavLink href="/events" className={drawerNavLink}>🎟️ Sự kiện</NavLink>
              {user && <NavLink href="/tickets" className={drawerNavLink}>🎫 Vé của tôi</NavLink>}
              {user && (
                <NavLink href="/notifications" className={drawerNavLink}>
                  🔔 Thông báo
                  {unread > 0 && (
              ...
              {user && <NavLink href="/profile" className={drawerNavLink}>👤 Hồ sơ</NavLink>}
              {user && (
                <Link href="/feedback" onClick={() => setDrawerOpen(false)}
              ...
              {user && (user.roles?.includes('ADMIN') || user.roles?.includes('ORGANIZER')) && (
                <>
                  <NavLink href="/admin/events" className={drawerNavLink}>🛠 Quản trị sự kiện</NavLink>
                  <NavLink href="/admin/analytics" className={drawerNavLink}>📊 Báo cáo</NavLink>
                  <NavLink href="/admin/feedback" className={drawerNavLink}>📋 Phản hồi KH</NavLink>
                </>
              )}
            </nav>
            <div className="mt-auto pt-4 border-t border-line">
              {user ? (
                ...
                <div className="space-y-2">
                  <Link href="/login" onClick={() => setDrawerOpen(false)} className="btn-ghost w-full justify-center">
                    Đăng nhập
                  </Link>
                  <Link href="/register" onClick={() => setDrawerOpen(false)} className="btn-primary w-full justify-center">
                    Tạo tài khoản
                  </Link>
                </div>
              )}
