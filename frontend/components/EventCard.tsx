  ...
  return (
    ...
    >
      <div className={`relative ${isLarge ? 'aspect-[21/9]' : 'aspect-[16/10]'} bg-brand-100 overflow-hidden`}>
        ...
        <span
          className={`absolute top-3 left-3 px-2.5 py-1 rounded-full text-xs font-semibold bg-white shadow-sm ${cat.tint}`}
        >
        ...
        {badge && (
          <span
            className={`absolute top-3 right-3 px-2.5 py-1 rounded-full text-[10px] font-bold tracking-wide ${badge.cls}`}
          >
            {badge.label}
          </span>
        )}
      ...
      <div className="p-4 flex flex-col flex-1">
        ...
        {event.organizer && <div className="text-xs text-ink-subtle mt-1">Tổ chức: {event.organizer}</div>}
        <div className="mt-3 pt-3 border-t border-line/70 flex items-end justify-between">
          ...
          {event.totalSeats != null && event.availableSeats != null && (
            <div
              className={`text-xs ${badge && badge.label === 'SẮP HẾT' ? 'text-warn-700 font-semibold' : 'text-ink-subtle'}`}
            >
