...
function FilterRow({ active, onClick, icon, label, count }) {
  return (
    <button
      onClick={onClick}
  ...
  return (
    <div className="mb-3">
      ...
      <div className="space-y-2">
        {items.map((it) => (
          <NotificationItem key={it.id} item={it} onClick={() => onClick(it)} />
        ))}
      </div>
    </div>
  );
...
function NotificationItem({ item, onClick }) {
  ...
  if (item.linkUrl) {
    return <Link to={item.linkUrl} onClick={onClick}>{content}</Link>;
  }
