          ...
          {featured ? (
            <>
              ...
              <div className="mt-5 sm:mt-6 flex flex-col sm:flex-row flex-wrap gap-3">
                <Link to={`/events/${featured.id}`} className="btn-primary justify-center">Đặt vé ngay →</Link>
                <Link to={`/events/${featured.id}`} className="btn-outline justify-center">Xem chi tiết</Link>
              </div>
            </>
          ) : (
...
function SectionHeader({ title, subtitle, linkTo }) {
  return (
    <div className="flex items-end justify-between">
      ...
      {linkTo && (
        <Link to={linkTo} className="text-sm font-semibold text-brand-700 hover:underline">
          Xem tất cả →
        </Link>
      )}
    </div>
