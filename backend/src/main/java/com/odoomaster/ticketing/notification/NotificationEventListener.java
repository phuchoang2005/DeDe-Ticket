 ...
 *
 * <p>
 * Uses a synchronous {@link EventListener}: it runs in the publisher's thread
 * and joins the
 * same transaction as {@code OrderService.pay()}, so notification creation has
 * exactly the same
 * timing/atomicity as the previous direct call. If notifications later need to
 * be sent only after
