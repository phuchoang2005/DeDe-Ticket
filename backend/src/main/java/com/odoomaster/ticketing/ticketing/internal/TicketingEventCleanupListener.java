...
/**
 * Purges ticketing rows for a deleted event: the check-ins for its tickets,
 * then the tickets. Observes
 * the catalog's {@link EventDeletedEvent} synchronously so this runs inside the
 * delete transaction
 * (the whole cascade stays atomic), keeping the {@code Ticket}/{@code CheckIn}
 * entities private to the
 * module. Check-ins are removed first to satisfy the
 * {@code check_ins.ticket_id → tickets.id} FK — the
 * old catalog-side cascade deleted only tickets, so deleting an event with
 * checked-in tickets would
