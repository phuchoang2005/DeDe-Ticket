...
/**
 * Purges sales rows for a deleted event: its order items, payments, and orders.
 * Observes the
 * catalog's {@link EventDeletedEvent} synchronously so this runs inside the
 * delete transaction (the
 * whole cascade stays atomic), keeping sales' {@code Order}/{@code Payment}
