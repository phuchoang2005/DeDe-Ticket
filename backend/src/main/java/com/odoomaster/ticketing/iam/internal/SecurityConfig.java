...
/**
 * Central Spring Security configuration: stateless JWT auth, CORS, and route
 * authorization.
 *
 * <p>
 * Sessions are stateless and CSRF is disabled (token auth). Public routes are
 * {@code /v1/auth/**}, {@code /v1/health}, and {@code GET /v1/events/**};
 * {@code /v1/admin/**}
 * requires role {@code ADMIN} or {@code ORGANIZER}; everything else requires
 * authentication.
 * The {@link JwtAuthenticationFilter} runs before the username/password filter,
 * and auth/access
 * failures are rendered as the standard {@link ApiErrorEnvelope}.
...
public class SecurityConfig {
  ...
  /**
   * @return permissive CORS for all origins (token auth, no credentials),
   *         exposing {@code X-Request-Id}
   */
  @Bean
  ...
  /**
   * Define the security filter chain: route rules, stateless sessions, JSON
   * auth/access
   ...
   */
  @Bean
  public SecurityFilterChain filterChain(HttpSecurity http, JwtAuthenticationFilter jwtFilter, ObjectMapper mapper)
      throws Exception {
    http
        .cors(c -> {
        })
