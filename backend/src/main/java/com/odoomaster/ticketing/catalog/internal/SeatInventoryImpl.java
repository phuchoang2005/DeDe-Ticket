...
/**
 * Catalog-owned implementation of {@link SeatInventory}: the concurrency crux of the system now
 * lives here rather than in {@code sales}' {@code OrderService}.
 *
 * <p>Holds the {@code AVAILABLE → LOCKED → SOLD} state machine, the {@value #LOCK_TTL_MINUTES}-minute
 * lock TTL, and the event-cache eviction that must accompany any seat mutation. Mutating methods are
 * {@code @Transactional}, so when called from within the ordering transaction they join it (the sale
 * stays atomic) and their cache evictions — registered against the transaction-aware cache manager —
 * fire on commit.
 */
@Service
public class SeatInventoryImpl implements SeatInventory {
    ...
    /**
     * Evict the event's seat-map and detail caches (by key) and clear the events list. Colocated with
     * the seat mutations so availability is never read stale — the responsibility that used to live in
