...
public class DataSeeder implements CommandLineRunner {
    ...
    private static String[] sectionNamesFor(String category) {
        if (category.contains("Thể thao")) return new String[]{"Khán đài A", "Khán đài B", "Khán đài C"};
        if (category.contains("Workshop")) return new String[]{"Workshop", "Mở rộng"};
        if (category.contains("Festival")) return new String[]{"General", "Front Stage"};
        if (category.contains("Seminar")) return new String[]{"Standard", "Premium"};
        if (category.contains("Nghệ thuật")) return new String[]{"Tầng 1", "Tầng 2", "Lô"};
